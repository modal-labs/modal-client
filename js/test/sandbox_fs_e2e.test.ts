...
/**
 * Maximum chunk size (bytes) for stdin writes that go through the task command
 * router used by `Sandbox.exec`.
 */
const TASK_COMMAND_ROUTER_MAX_BUFFER_SIZE = 16 * 1024 * 1024;
...
async function readRemoteFile(sb: Sandbox, path: string): Promise<Uint8Array> {
...
async function mkdirRemote(sb: Sandbox, path: string): Promise<void> {
  const p = await sb.exec(["mkdir", "-p", path]);
  const rc = await p.wait();
  if (rc !== 0) throw new Error(`mkdirRemote failed for ${path} (rc=${rc})`);
}

async function pathExists(sb: Sandbox, path: string): Promise<boolean> {
  const p = await sb.exec(["test", "-e", path]);
  return (await p.wait()) === 0;
}

async function isDirRemote(sb: Sandbox, path: string): Promise<boolean> {
...
test("SandboxFsE2eWriteBytesLargeFileExceedingStdinBufferLimit", async () => {
  const sb = await newTestSandbox();
  const payload = randomBytes(TASK_COMMAND_ROUTER_MAX_BUFFER_SIZE + 1024, 40);
