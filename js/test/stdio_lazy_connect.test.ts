...
async function startFakeWorker(chunks?: Uint8Array[]) {
  const reads: TaskExecStdioFileDescriptor[] = [];
  const server = createServer();
  server.add(
    TaskCommandRouterDefinition,
    fakeWorker(reads, chunks ?? [new TextEncoder().encode(OUTPUT)]) as any,
  );
...
test("binary mode hands back the bytes unchanged", async () => {
  ...
  try {
    ...
    expect(await process.stdout.readBytes()).toEqual(
