...
def _parameter_tag(param: ParsedParam, body: str = "") -> str:
    ...
    attrs = f'name="{name_esc}" type="{type_esc}"{default_attr} description="{desc_esc}"'
    if body:
...
def _unwrap_synchronicity_class(obj):
    """Return the implementation class behind a synchronicity wrapper, or `obj` unchanged.

    `synchronize_api` builds the public class from scratch, so a wrapper around a
    dataclass carries neither `__dataclass_fields__` nor a source file to read
    annotations from; both live on the implementation class.

    A hand-written subclass of a wrapper inherits the synchronizer but is not itself a
    wrapper, and `_translate_in` renders it as the plain class it is: the unwrapping it
    does is keyed on the wrapper's own `__dict__`.
    """
    synchronizer = getattr(obj, SYNCHRONIZER_ATTR, None)
...
def _dataclass_attributes(name, obj, docstring: str) -> list[ParsedParam]:
    ...
    for field in fields:
        ...
        if object_is_private(field.name, field):
            continue
...
def class_str(
    ...
    _ancestors: frozenset[type] = frozenset(),
    show_attributes_tag: bool = True,
):
    def qual_name(cls):
    ...
    # Dataclass rendering needs fields and annotation source, which live on the
    # implementation class rather than on a synchronicity wrapper of it.
    impl_obj = _unwrap_synchronicity_class(obj)
    if impl_obj in _ancestors:
        return ""
    ancestors = _ancestors | {impl_obj}
    is_dataclass = dataclasses.is_dataclass(impl_obj)
...
```\n\n"""
    ...
    elif is_dataclass and _has_autogenerated_dataclass_doc(impl_obj):
        docstring = ""
    else:
        docstring = clean_docstring(obj.__doc__)
    ...
    if class_doc_markdown:
        header.append(class_doc_markdown + "\n")

    parts: list[str] = []
    inline_classes: set[type] = set()
    ...
    if isinstance(obj, EnumMeta):
        ...
        attributes = _dataclass_attributes(name, impl_obj, docstring)
        if attributes:
            if show_attributes_tag:
                parts.append("**Attributes**\n\n")

            namespace = {key: value for cls in reversed(impl_obj.__mro__) for key, value in vars(cls).items()}
            namespace[impl_obj.__name__] = impl_obj
            nested_classes = {
                value
                for key, value in namespace.items()
                if inspect.isclass(value) and not object_is_private(key, value)
            }
            fields = {field.name: field for field in dataclasses.fields(impl_obj)}

            def get_referenced_classes(annotation):
                if inspect.isclass(annotation) and annotation in nested_classes:
                    yield annotation
                for arg in typing.get_args(annotation):
                    yield from get_referenced_classes(arg)

            for attr in attributes:
                # Resolve each field separately so one unknown forward reference cannot hide the others.
                def field_type():
                    pass

                field_type.__annotations__ = {"return": fields[attr.name].type}
                module = inspect.getmodule(impl_obj)
                try:
                    annotation = typing.get_type_hints(
                        field_type, globalns=vars(module) if module else {}, localns=namespace
                    )["return"]
                except (NameError, TypeError):
                    annotation = fields[attr.name].type

                definitions = []
                referenced_classes = dict.fromkeys(get_referenced_classes(annotation))
                for nested in referenced_classes:
                    inline_classes.add(nested)
                    body = class_str(
                        nested.__name__,
                        nested,
                        title_level + "#",
                        _ancestors=ancestors,
                        show_attributes_tag=False,
                    )
                    if body:
                        if len(referenced_classes) > 1:
                            definitions.append(f"**{nested.__name__}**\n\n{body}")
                        else:
                            definitions.append(f"\n\n{body}")
                parts.append(_parameter_tag(attr, "\n".join(definitions)))
                parts.append("\n")
            parts.append("\n")
    ...
    nested_class_definitions: list[str] = []
    ...
    for member_name, member in entries.items():
        ...
        if inspect.isclass(member):
            if member not in inline_classes and _unwrap_synchronicity_class(member) not in ancestors:
                title = _escape_svelte_html_attr(member_name)
                body = class_str(member_name, member, member_title_level, _ancestors=ancestors)
                nested_class_definitions.append(f'<Collapsible title="{title}">\n{body}</Collapsible>\n\n')
            continue
    ...
    if nested_class_definitions:
        parts.extend(nested_class_definitions)
