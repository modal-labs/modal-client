...
- Added `modal environment roles list` and `modal environment roles update` CLIs for managing the Environment Roles of users and service users. The equivalent `modal environment members` commands are now deprecated in favor of these.
- Added the [`Environment.roles`](/docs/sdk/py/latest/Environment#roles) namespace (`list` and `update`) for managing the Environment Roles of users and service users. `Environment.members` is now a deprecated alias for it.
