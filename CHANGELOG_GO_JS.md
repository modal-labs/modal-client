...
- Added an **experimental** Sandbox sidecar API in the Go SDK via `sandbox.ExperimentalSidecars`. It exposes `Create`, `Get`, and `List` for managing additional named containers that run alongside the Sandbox's main container, plus a `SidecarContainer` handle with `Exec`, `Wait`, `Poll`, `Terminate`, and `Filesystem`. The API is subject to change.
