...
async def lifespan_ctx_manager(asgi_app):
    ...
    lm = modal._runtime._asgi.LifespanManager(asgi_app, state)
...
async def test_web_server_wrapper_immediate_disconnect(http_dummy_server: DummyHttpServer):
    proxy_asgi_app = modal._runtime._asgi.web_server_proxy(http_dummy_server.host, http_dummy_server.port)
