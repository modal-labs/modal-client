...
def test_persisted_nfs(servicer, client):
    # Lookup should fail since it doesn't exist
    with pytest.raises(NotFoundError):
        modal.Volume.lookup("xyz", client=client, nfs=True)

    # Create it
    modal.Volume.lookup("xyz", create_if_missing=True, client=client, nfs=True)
...
def test_invalid_name(servicer, client, name):
    with pytest.raises(InvalidError, match="Invalid Volume name"):
        modal.Volume.lookup(name, nfs=True)
...
def test_network_file_system_handle_single_file(client, tmp_path, servicer):
    ...
    with modal.Volume.ephemeral(client=client, nfs=True) as nfs:
...
async def test_network_file_system_handle_big_file(client, tmp_path, servicer, blob_server, *args):
    with mock.patch("modal.volume.LARGE_FILE_LIMIT", 10):
        local_file_path = tmp_path / "bigfile"
        local_file_path.write_text("hello world, this is a lot of text")

        async with modal.Volume.ephemeral(client=client, nfs=True) as nfs:
...
def test_nfs_ephemeral(servicer, client, tmp_path):
    ...
    with modal.Volume.ephemeral(client=client, _heartbeat_sleep=1, nfs=True) as nfs:
        assert nfs.listdir("/") == []
...
def test_nfs_lazy_hydration_from_name(set_env_client):
    nfs = modal.Volume.from_name("nfs", create_if_missing=True, nfs=True)
