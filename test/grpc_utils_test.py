...
async def test_retry_transient_errors(servicer, client):
    ...
    assert await wrapped_blob_create.aio(req, Retry(max_retries=None, base_delay=0))
    assert servicer.blob_create_metadata.get("x-idempotency-key")
    ...
    with pytest.raises(GRPCError):
        assert await wrapped_blob_create.aio(req, Retry(max_retries=None, total_timeout=3))
