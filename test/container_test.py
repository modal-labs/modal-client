...
def test_generator_success(unix_servicer, event_loop):
    ...
    assert len(ret.items) == 43  # The generator creates N outputs, and N is 42 from the autogenerated input
    ...
    for i in range(42):
        ...
        assert deserialize(result.data, ret.client) == i**2
...
def test_generator_failure(unix_servicer, event_loop):
    ...
    assert len(ret.items) == 6  # 5 successful outputs, 1 failure
    ...
    data = deserialize(last_result.data, ret.client)
...
def test_function_returning_generator(unix_servicer, event_loop):
    ...
    assert ret.items[-1].result.gen_status == api_pb2.GenericResult.GENERATOR_STATUS_COMPLETE
...
def test_cls_generator(unix_servicer, event_loop):
    ...
    assert ret.items[0].result.status == api_pb2.GenericResult.GENERIC_STATUS_SUCCESS
    assert ret.items[0].result.gen_status == api_pb2.GenericResult.GENERATOR_STATUS_INCOMPLETE
    assert ret.items[0].result.data == serialize(42**3)
    assert ret.items[1].result.status == api_pb2.GenericResult.GENERIC_STATUS_SUCCESS
    assert ret.items[1].result.gen_status == api_pb2.GenericResult.GENERATOR_STATUS_COMPLETE
