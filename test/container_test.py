...
def test_function_io_doesnt_inspect_args_or_return_values(monkeypatch, servicer):
    ...
    monkeypatch.setattr("modal.runtime._container_io_manager.MAX_OBJECT_SIZE_BYTES", 1e100)
...
def test_inputs_outputs_with_blob_id(servicer, client, monkeypatch):
    monkeypatch.setattr("modal.runtime._container_io_manager.MAX_OBJECT_SIZE_BYTES", 0)
    ret = _run_container(
        servicer,
        "test.supports.functions",
        "ident",
...
def test_container_heartbeat_survives_grpc_deadlines(servicer, caplog, monkeypatch):
    monkeypatch.setattr("modal.runtime._container_io_manager.HEARTBEAT_INTERVAL", 0.01)
    num_heartbeats = 0
...
def test_container_heartbeat_survives_local_exceptions(servicer, caplog, monkeypatch):
    ...
    monkeypatch.setattr("modal.runtime._container_io_manager.HEARTBEAT_INTERVAL", 0.01)
    monkeypatch.setattr(
        "modal.runtime._container_io_manager._ContainerIOManager._heartbeat_handle_cancellations", custom_heartbeater
    )
