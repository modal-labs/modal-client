...
- Added [`Function.Logs`](/docs/sdk/go/latest/Function#logs) and [`FunctionCall.Logs`](/docs/sdk/go/latest/FunctionCall#logs) APIs for fetching current and historical logs generated by that object. The `Logs` namespace exposes three methods: use [`Fetch`](/docs/sdk/go/latest/Function#logsfetch) for time-based queries, [`Tail`](/docs/sdk/go/latest/Function#logstail) for returning the most recent entries, or [`Stream`](/docs/sdk/go/latest/Function#logsstream) for yielding entries as they arrive.
- [`Sandbox.Filesystem.CopyFromLocal`](/docs/sdk/go/latest/Sandbox#filesystemcopyfromlocal) / [`WriteBytes`](/docs/sdk/go/latest/Sandbox#filesystemwritebytes) / [`WriteText`](/docs/sdk/go/latest/Sandbox#filesystemwritetext) now stream data to the Sandbox instead of issuing a request per chunk, making large writes ~2.5× faster.
