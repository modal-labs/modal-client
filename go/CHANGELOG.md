...
- The client now releases the connection to a Sandbox host when the connection is idle and automatically reconnects on subsequent operations, so it is no longer necessary to call [`Sandbox.Detach()`](/docs/sdk/go/latest/Sandbox#detach) for resource cleanup.

### 0.10.0 (2026-08-27)

- Added [`Function.Logs`](/docs/sdk/go/latest/Function#functionlogs) and [`FunctionCall.Logs`](/docs/sdk/go/latest/FunctionCall#functioncalllogs) APIs for fetching current and historical logs generated by that object. The `Logs` namespace exposes three methods: use [`Fetch`](/docs/sdk/go/latest/Function#fetch) for time-based queries, [`Tail`](/docs/sdk/go/latest/Function#tail) for returning the most recent entries, or [`Stream`](/docs/sdk/go/latest/Function#stream) for yielding entries as they arrive.
- [`Sandbox.Filesystem.CopyFromLocal`](/docs/sdk/go/latest/Sandbox#copyfromlocal) / [`WriteBytes`](/docs/sdk/go/latest/Sandbox#writebytes) / [`WriteText`](/docs/sdk/go/latest/Sandbox#filesystemwritetext) now stream data to the Sandbox instead of issuing a request per chunk, making large writes ~2.5× faster.
- [`Function.UpdateAutoscaler`](/docs/sdk/go/latest/Function#updateautoscaler) now returns a struct containing the current autoscaler settings immediately after applying the change.
