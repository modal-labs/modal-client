...
async def _iter_stream_as_bytes(stream: _StreamReader[T]):
    ...
    async for part in stream:  # type: ignore[func-returns-value]
...
class _ContainerProcessThroughCommandRouter(Generic[T]):
    ...
    async def poll(self) -> Optional[int]:
        # TODO(saltzm): Implement this using a new RPC.
        raise NotImplementedError("poll is not implemented for _ContainerProcessThroughCommandRouter")
    ...
    async def wait(self) -> int:
        ...
        while True:
            ...
            # Poll with a short timeout to avoid blocking for too long in the event of a
            # network issue or server bug that would cause the wait to hang.
            rpc_timeout = 60 if self._exec_deadline is None else int(min(10, max(1, int(remaining))))
            resp = await self._router_client.exec_wait(self._task_id, self._process_id, timeout=rpc_timeout)
            if resp is None:
                # RPC timed out; loop to re-check deadline and continue waiting
                continue
