...
class _ContainerProcessThroughServer(Generic[T]):
    ...
    async def attach(self):
        ...
        async def _write_to_fd_loop(stream: _StreamReader):
            # TODO(saltzm): This is required to make modal shell to an existing task
            # work, since that uses ContainerExec RPCs directly, but this is hacky.
            # Once we use the new exec path for that use case, we can clean this up.
