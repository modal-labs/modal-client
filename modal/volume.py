    ...
    async def commit(self):
        ...
        async with await self._get_lock():
            ...
            try:
                ...
                resp = await self._client.stub.VolumeCommit(req, retry=RetryRPC(max_retries=90))
    ...
    async def copy_files(self, src_paths: Sequence[str], dst_path: str, recursive: bool = False) -> None:
        ...
        **Usage**
        ...
        if self._is_v1:
            ...
            await self._client.stub.VolumeCopyFiles(request, retry=RetryRPC(base_delay=1))
        else:
            request = api_pb2.VolumeCopyFiles2Request(
                volume_id=self.object_id, src_paths=src_paths, dst_path=dst_path, recursive=recursive
            )
            await self._client.stub.VolumeCopyFiles2(request, retry=RetryRPC(base_delay=1))
...
class _VolumeUploadContextManager(_AbstractVolumeUploadContextManager):
    ...
    async def __aexit__(self, exc_type, exc_val, exc_tb):
        if not exc_val:
            ...
            try:
                await self._client.stub.VolumePutFiles(request, retry=RetryRPC(base_delay=1))
            except GRPCError as exc:
    ...
    async def _upload_file(self, file_spec: FileUploadSpec) -> api_pb2.MountFile:
        ...
        response = await self._client.stub.MountPutFile(request, retry=RetryRPC(base_delay=1))
        ...
        if not response.exists:
            ...
            while (time.monotonic() - start_time) < VOLUME_PUT_FILE_CLIENT_TIMEOUT:
                response = await self._client.stub.MountPutFile(request2, retry=RetryRPC(base_delay=1))
                if response.exists:
                    break
