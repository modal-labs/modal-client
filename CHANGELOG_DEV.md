...
- `Sandbox.create` now waits for Sandboxes requesting GPUs to be scheduled before returning, for up to 21 minutes. If no capacity becomes available within that time, the Sandbox is terminated and `modal.exception.ResourceExhaustedError` is raised.
