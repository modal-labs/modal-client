...
## Go
- Added a `logs` attribute for `Sandbox` objects for fetching current and historical logs generated by that object. The `logs` propery exposes a `fetch` method for time-based queries and `tail` method for fetching the most recent entries.
